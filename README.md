...
| CORE-012 | El histórico dependía de `tail` y Perl dentro de zsh y sólo conocía sesiones Claude, por lo que en PowerShell podía cargar indefinidamente y no podía restaurar el harness original. | Lector JSONL nativo en Rust e indexadores/parsers/adaptadores completos para Claude Code, Codex CLI y Kimi Code; cada sesión conserva proveedor, cwd, configuración y modelo. | Pruebas de formatos, quoting PowerShell/POSIX y suite Rust; contrato en [Historial multiproveedor](docs/HISTORIAL-MULTIPROVEEDOR.md). |
